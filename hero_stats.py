...
class HeroStats:
    ...
    def find_hero_winrate(self, index):
        ...
        return [hero_name, hero_winrate]

    def add_hero(self, index):
        [hero_name, hero_winrate] = self.find_hero_winrate(index)
        if len(self.top_three_heroes) < 3:
            ...
            if hero_winrate > min_value[1]:
                ...
                self.top_three_heroes[min_index] = [hero_name, hero_winrate]

    def ban_calculator(self):
        for i in range(len(self.heroes)):
            self.add_hero(i)

        for hero, winrate in self.top_three_heroes:
            if winrate > 59.9:
                self.ban_heroes.append([hero, winrate])
